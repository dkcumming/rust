...
pub fn write_mir_json<'tcx>(
    tcx: TyCtxt<'tcx>,
    single: Option<DefId>,
    w: &mut dyn io::Write,
) -> io::Result<()> {
    // writeln!(w, "// WARNING: JSON")?;
    ...
    //     }
        ...
        let render_body = |w: &mut dyn io::Write, body| -> io::Result<()> {
            // json_mir_fn(tcx, body, &mut |_, _| Ok(()), w)?;
            writeln!(w, "{}", json!(body));
        ...
        if tcx.is_const_fn_raw(def_id) {
            ...
            // // json_mir_fn(tcx, tcx.mir_for_ctfe(def_id), &mut |_, _| Ok(()), w)?;
        } else {
...
// pub fn json_mir_intro<'tcx>(
//     tcx: TyCtxt<'tcx>,
//     body: &Body<'_>,
//     w: &mut dyn io::Write,
// ) -> io::Result<()> {
//     json_mir_sig(tcx, body, w)?;
//     // writeln!(w, "{{")?;
...
// impl Serialise for Body

// fn json_mir_sig(tcx: TyCtxt<'_>, body: &Body<'_>, w: &mut dyn io::Write) -> io::Result<()> {
//     // use rustc_hir::def::DefKind;

//     trace!("write_mir_sig: {:?}", body.source.instance);
//     let def_id = body.source.def_id();
//     let _kind = tcx.def_kind(def_id);
//     // write!(w, "{}", tcx.def_descr(def_id))?;
//     // write!(w, "{}", tcx.def_kind_descr(kind, def_id))?;

//     // Daniel: this is more a top level thing, it doesn't need to happen here
//     write!(w, "{}", json!({"DefId": { "Index": def_id.index.as_u32(), "Krate": def_id.krate.as_u32()}}))?;
